...
new class extends Component
{
    ...
    public function exception($e, $stopPropagation)
    {
        if ($e instanceof ValidationException) {
            $this->dispatch('errors-user-save', errors: $this->getErrorBag());
    ...
    {
        return $this->editing ? 'Editar Usuário' : 'Cadastrar Usuário';
    }
    ...
    public function submit()
    {
        ...
        try {
            $this->user = DB::transaction(function () {
                $data = [
                ...
                if ($auth->role === 'customer') {
                    $auth->internalUsers()->attach($user->id, [
                        'id' => (string) Str::ulid(),
                    ]);
                } else {
                    $customer = $auth->linkedCustomers()->firstOrFail();

                    $customer->internalUsers()->attach($user->id, [
                        'id' => (string) Str::ulid(),
                    ]);
                }

                session()->flash('success', 'Usuário criado com sucesso, ele receberá um email em breve para definir a senha.');
            ...
            Log::channel('user')->error('Erro ao salvar usuário', [
        ...
        return [
            ...
            'form.email' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if ($user && $user->isNot($this->user)) $fail(__('validation.unique'));
                }
            ],
            'form.cpf_cnpj' => [
                'required',
...
<div class="flex-1 flex flex-col">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-5 mb-3">
        <div class="flex items-center gap-4">
            <a href="{{ route('panel.users.index') }}" wire:navigate class="inline-flex items-center justify-center rounded-md border border-[#394150]/30 bg-[#394150]/5 px-3 py-2 text-text-soft transition hover:bg-surface-hover">
    ...
    <div class="flex flex-col-reverse gap-4 md:flex-row md:items-center md:justify-between mt-3">
        <a href="{{ route('panel.users.index') }}" wire:navigate class="text-center text-[11px] uppercase tracking-wide text-text-soft/50 transition hover:text-text-soft"><i class="las la-angle-left text-xs"></i> Voltar</a>
    </div>
